...
class ResNet164_v2(nn.HybridBlock):
    def __init__(self, num_classes, verbose=False, **kwargs):
        ...
        with self.name_scope():
           ...
           # net.add(nn.Dropout(0.5))
            for _ in range(26):
                net.add(Residual_v2_bottleneck(channels=128))rc/operator/././cudnn_algoreg-inl.h:106: Running performance tests to find the best convolution algorithm, this can take a while... (setting env variable MXNET_CUDNN_AUTOTUNE_DEFAULT to 0 to disable)
...
def train(net, train_data, valid_data, num_epochs, lr, wd, ctx, lr_period, lr_decay):
    ...
    for epoch in range(num_epochs):
        ...
        if((valid_data is not None) and (valid_acc>0.05)):
            break
        if((valid_data is None) and ((train_acc / len(train_data))>0.1)):
            break
ctx = utils.try_gpu()
num_epochs = 50
learning_rate = 0.1
