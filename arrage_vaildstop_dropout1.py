...
def train(net, train_data, valid_data, num_epochs, lr, wd, ctx, lr_period, lr_decay):
    ...
    for epoch in range(num_epochs):
        ...
        if((valid_data is not None) and (valid_acc>0.92)):
            break
        if((valid_data is not None) and (epoch>1000)):
            break
...
df.to_csv('submissi111onvaild.csv', index=False)
...
df1.to_csv('submissi111onfull.csv', index=False)
